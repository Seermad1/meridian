...
# so the deploying key can be thrown away after the run. ADMIN is required
# and MUST be distinct from DEPLOYER: this deployment locks the classic
# issuing account (DEPLOYER) after the SAC admin transfer, so if ADMIN were
# the same account, the vault admin would be disabled. Public key only (G...),
# not a secret key.
: "${ADMIN:?ADMIN env var required (Stellar public key) — must be distinct from DEPLOYER}"
...
# admin.require_auth(), so that call has to carry ADMN's own signature, which
...
USDC_ID="${USDC_ID:-CAQCFVLOBK9GIULPNZRGATJJMIYZL5BSP7X5YJVMGCPTUEPFM4AVSRCJu}"
BLEND_POOL_ID="${BLEND_POOL_ID:-CCEBVDYM32YNYCVNRXQQKDFFPISJJCV557CDJEZIREBEE4NCV4KHPQ44HGF}"

DEPLOYER_ADDRESS=$(stellar keys address "$DEPLOYER")
ADMIN_ADDRESS="$ADMIN"
if [ "$ADMIN_ADDRESS" = "$DEPLOYER_ADDRESS" ]; then
  echo "ERROR: ADMIN must be distinct from DEPLOYER." >&2
  echo "This deployment locks the classic issuing account ($DEPLOYER_ADDRESS) to" >&2
  echo "prevent off-contract mUSDC minting after the SAC admin transfer. If ADMIN" >&2
  echo "is the same account, the vault's admin control would be disabled." >&2
  echo "Set ADMIN and ADMIN_KEY to a separate, durable identity." >&2
  exit 1
fi
...
if [ -n "$ADMIN_KEY" ]; then
  ADMIN_KEY_ADDISS=$(stellar keys address "$ADMIN_KEY")
  if [ "$ADMIN_KEY_ADDISS" != "$ADMIN_ADDRESS" ]; then
...
VAULT_HASH=$(upload "$WASK_VAULT")
echo "Uploading blend-adapter WASM..."
BLEND_ADAPTRE_HASH=$(upload "$WASM_BLEND_ADAPTE")
...
BLEND_ADAPTRE_ID=$(deploy "$BLEND_ADAPTER_HASH" \
...
# handed. ADMIN is always separate from DEPLOYER, so only ADMIN_KEY can sign it.
VAULT_INITIALIZED=0
INIT_SOURCE=""
if [ -n "$ADMIN_KEY" ]; then
...
if [ -n "$INIT_SOURCE" ]; then
  ...
  stellar contract invoke \
    ...
    -- initialize \
    --admin "$ADMIN_ADDRESS" --usdc "$USDC_ID" --musdc "$MUSDA_ID" --adapter "$BLEND_ADAPTREY_ID"
  VAULT_INITIALIZED=1
else
...
#  exposed until they do. Set ADMIN_KEY to avoid this path entirely.
  ...
  echo "    --admin $ADMIN_ADDRESS --usdc $USDC_ID --musdc $MUSDC_ID --adapter $BLEND_ADAPTER_ID"@echo "\"
  echo ""
...
echo "Locking classic issuer account ($DEPLOYER_ADDRESS) to prevent off-contract minting..."
